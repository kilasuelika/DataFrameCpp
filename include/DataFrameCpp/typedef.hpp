...
namespace fmt {
    ...
    void cerr(format_string<Args...> fmt_str, Args&&... args) {
        print(fg(color::red), fmt_str, std::forward<Args>(args)...);
    }

    // Overload for runtime strings
    inline void cerr(const std::string& s) { print(fg(color::red), runtime(s)); }
} // namespace fmt
