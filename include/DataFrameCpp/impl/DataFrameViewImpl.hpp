...
namespace dfc {
    ...
    template<typename T>
    inline DataFrameView& DataFrameView::set(const std::string& colname, const T& v) {
        auto col = _column_map.find(colname);
        if (col != _column_map.end()) {
            _values[col->second]->set(v);
        }
        else {
            std::cerr << std::format("Column {} not found.", colname) << std::endl;
        }
        return *this;
    }
