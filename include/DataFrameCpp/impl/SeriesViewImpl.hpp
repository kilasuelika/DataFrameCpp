...
namespace dfc {
    inline SeriesView::SeriesView(Series* series)
        : _index(std::make_shared<Int64RangeIndex>(series->size())), _values(series) {
    }

    inline dfc::SeriesView::SeriesView(Series* series, const std::vector<size_t>& pos)
        : _values(series), _index(std::make_shared<Index>(pos)) {
    }

    inline SeriesView::SeriesView(Series* series, const std::vector<long long>& pos)
        : _values(series), _index(std::make_shared<Index>(series->_cal_index(pos))) {
    }

    inline dfc::SeriesView::SeriesView(const SeriesView* view, const std::vector<size_t>& pos)
        : _values(view->_values), _index(std::make_shared<Index>(view->_index, pos)) {
    }
...
#define DataFrameCpp_SeriesView_Scalar_Compare_Case_level1(dtypename, type, op)                    \
    case dtypename: {                                                                              \
        _Pragma("omp parallel for") DataFrameCpp_SeriesView_Scalar_Compare_Transform(type, op);    \
        break;                                                                                     \
    }
...
#define DataFrameCpp_SeriesView_Scalar_Compare(op)                                                 \
    template <supported_type S> Series operator op(const SeriesView &lhs, const S &rhs) {          \
        ...
        if constexpr (arithmetic_type<S>) {                                                        \
            switch (lhs.dtype()) {                                                                 \
                DataFrameCpp_SeriesView_Scalar_Compare_Case_level1(DType::BOOL, bool, op);                \
                DataFrameCpp_SeriesView_Scalar_Compare_Case_level1(DType::INT, int, op);                  \
                DataFrameCpp_SeriesView_Scalar_Compare_Case_level1(DType::LONGLONG, long long, op);       \
                DataFrameCpp_SeriesView_Scalar_Compare_Case_level1(DType::FLOAT, float, op);              \
                DataFrameCpp_SeriesView_Scalar_Compare_Case_level1(DType::DOUBLE, double, op);            \
            default:                                                                               \
                break;                                                                             \
            }                                                                                      \
