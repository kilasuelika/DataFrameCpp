...
inline dfc::Series::Series(size_t n)
    : _size(n) {
}
...
inline Series::Series(const std::string &name, DType type, size_t sz)
    : _name(name), _size(sz) {
    switch (type) {
    case DType::STRING:
...
inline dfc::Series::Series(const SeriesView &view)
    : _name(view.name()), _size(view.size()) {
    std::visit(
    ...
    if (dtype() == DType::NONE) {
        ...
        if constexpr (std::is_same_v<T, std::string>) {
            ...
            if (is_arithmetic(_values.index())) {
                ...
                // From date_time
                if (_values.index() == DType::DATEDURATION) {
                    std::vector<T> res(_size);
            ...
            } else if (_values.index() == DType::INT) {
...
#define DataFrameCpp_Series_Series_Compare_Transform(type, op)                       \
    auto &lhs_v = lhs.vector<type>();                  \
        ...
        std::transform(std::execution::par, lhs_v.cbegin(), lhs_v.cend(), rhs_v.cbegin(), res_v.begin(),                    \
                   [](const type &e1, const type &e2) -> bool { return e1 op e2; })
...
inline Series dfc::operator op(const Series &lhs, const Series &rhs) {\
    ...
    if (is_arithmetic(lhs.dtype()) && is_arithmetic(rhs.dtype())) {\
        ...
        std::string info = std::format("Can't compare between {}({}) and {}({}).\n", lhs.name(),\
...
#define DataFrameCpp_Series_Scalar_Compare_Transform(type, op)                                 \
    auto &lhs_v = lhs.vector<type>();                                                              \
    std::transform(                                                                                \
        std::execution::par, lhs_v.cbegin(), lhs_v.cend(),                                         \
        res_v.begin(),[&](const type &l) { return l op rhs; })
...
#define DataFrameCpp_Series_Scalar_Compare(op)                                                     \
 ...
 inline Series operator op(const Series &lhs, const S &rhs) {\
       const auto rdtype = DType_traits<S>::dtype;                                                     \
        ...
        if constexpr (arithmetic_type<S>) {                            \
            ...
            std::string info =                                                                     \
                std::format("Can't compare between {}({}) and scalar type().\n", lhs.name(),              \
                            lhs.dtype_name(),DType_traits<S>::name);                       \
            fmt::cerr(info);                                                                       \
            throw(std::runtime_error(info));                                                       \
        }                                                                                          \
        return res;                                                                                \
    }                                                                  template<supported_type S>                            \
   inline Series operator op(const S &rhs, const Series &lhs) {       \
        return lhs op rhs;                                                                         \
    }
...
#define DataFrameCpp_Series_Scalar_Bitwise_Bin_Op(op)                                              \
  ...
  inline Series operator op(const Series &lhs, const S &rhs) {                                 \
    ...
    Series operator op(const S &rhs, const Series &lhs) {          \
        return lhs op rhs;                                                                         \
    }
